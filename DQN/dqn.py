...
# Code for Dueling DQN
# class MLP(nn.Module):
#     def __init__(self, state_dim, action_dim, hidden_dim=256):
#         super(MLP, self).__init__()
#         self.fc1 = nn.Linear(state_dim, hidden_dim)  # input layer
#         self.fc_value = nn.Linear(hidden_dim, hidden_dim)  # hidden layer for value stream
#         self.fc_advantage = nn.Linear(hidden_dim, hidden_dim)  # hidden layer for advantage stream
#         self.value = nn.Linear(hidden_dim, 1)  # output layer for state value
#         self.advantage = nn.Linear(hidden_dim, action_dim)  # output layer for advantage values

#     def forward(self, x):
#         x = F.relu(self.fc1(x))  # activation function
#         value = F.relu(self.fc_value(x))  # value stream
#         advantage = F.relu(self.fc_advantage(x))  # advantage stream
#         value = self.value(value)
#         advantage = self.advantage(advantage)
#         qval = value + (advantage - advantage.mean(dim=1, keepdim=True))  # combine value and advantage streams
#         return qval
