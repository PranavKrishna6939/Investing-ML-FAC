...
train_tickers = ['ACN', 'ABT', 'AMZN', 'BAC']
test_tickers = ['GOOG', 'NVDA', 'CAT', 'CSCO', 'KO'] 
...
class Config:
    ...
    def __init__(self):
        ################################## env hyperparameters ###################################
        self.algo_name = 'QRDQN' # algorithmic name
        ...
        self.num_quantiles = 51  # number of quantiles for QR-DQN
        ################################################################################
...
def env_agent_config(data, cfg, mode):
    ...
    agent = QRDQN(cfg.state_space_dim, cfg.action_space_dim, cfg)
...
if __name__ == "__main__":
    ...
    ax.bar(x, rewards, width=width, color='salmon', label='QR-DQN')
